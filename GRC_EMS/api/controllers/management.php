...
// Map each management module to the RBAC permission required to use it.
...
// Read and validate one required text field with a consistent error response.
...
// Supply dropdown data for management forms. Teacher choices are restricted to
// active users whose database role is Teacher.
function management_options(PDO $pdo): array {
    return [
        ...
        'teachers'=>$pdo->query("SELECT u.id,u.email,COALESCE(NULLIF(TRIM(CONCAT_WS(' ',up.first_name,up.middle_name,up.last_name)),''),u.username) name FROM users u JOIN roles r ON r.id=u.role_id LEFT JOIN user_profiles up ON up.user_id=u.id WHERE r.role_name='Teacher' AND u.status='active' ORDER BY name,u.email")->fetchAll(),
...
// List data for each CRUD module. JOINs include readable related information.
function management_rows(PDO $pdo,string $module): array {
    switch($module){
        ...
        case 'schedules': return $pdo->query("SELECT sc.id,sc.section_id,sc.subject_id,sc.teacher_user_id,sc.day_of_week,TIME_FORMAT(sc.start_time,'%H:%i') start_time,TIME_FORMAT(sc.end_time,'%H:%i') end_time,sc.room,COALESCE(NULLIF(TRIM(CONCAT_WS(' ',up.first_name,up.middle_name,up.last_name)),''),u.username) instructor,se.section_name,su.subject_code,su.subject_name FROM class_schedules sc JOIN sections se ON se.id=sc.section_id JOIN subjects su ON su.id=sc.subject_id JOIN users u ON u.id=sc.teacher_user_id JOIN roles r ON r.id=u.role_id AND r.role_name='Teacher' AND u.status='active' LEFT JOIN user_profiles up ON up.user_id=u.id ORDER BY FIELD(sc.day_of_week,'Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'),sc.start_time")->fetchAll();
        ...
        case 'users': return $pdo->query("SELECT u.id,u.username,u.email,u.role_id,u.status,u.must_change_password,u.created_at,r.role_name,(SELECT GROUP_CONCAT(se.section_name ORDER BY se.section_name SEPARATOR ', ') FROM teacher_sections ts JOIN sections se ON se.id=ts.section_id WHERE ts.teacher_user_id=u.id) assigned_sections FROM users u JOIN roles r ON r.id=u.role_id WHERE r.role_name IN ('Admin','Staff','Teacher','Registrar') ORDER BY r.role_name,u.email")->fetchAll();
        default: return [];
    }
}

// Bill items are child records of one assessment. Their active total is the
// assessment total shown to students and used by receipt verification.
function recalculate_assessment_total(PDO $pdo,int $assessmentId): void {
    $q=$pdo->prepare('SELECT COALESCE(SUM(amount),0) FROM assessment_bills WHERE assessment_id=? AND is_active=1');
    $q->execute([$assessmentId]);$total=(float)$q->fetchColumn();
    $pdo->prepare('UPDATE assessments SET amount=?,amount_paid=LEAST(amount_paid,?) WHERE id=?')->execute([$total,$total,$assessmentId]);
}

function assessment_bills(PDO $pdo): array {
    return $pdo->query("SELECT b.id,b.assessment_id,b.bill_code,b.bill_name,b.amount,b.due_date,b.notes,b.is_active,ass.description assessment_description,ass.school_year,ass.semester,sp.student_number,CONCAT(a.last_name,', ',a.first_name) student_name FROM assessment_bills b JOIN assessments ass ON ass.id=b.assessment_id JOIN student_profiles sp ON sp.id=ass.student_profile_id JOIN applications a ON a.id=sp.application_id ORDER BY b.created_at DESC,b.id DESC")->fetchAll();
}

/**
 * Server-side CRUD controller for records managed by Admin/Registrar/Staff.
 * Browser scripts only collect form values; this controller validates them,
 * checks foreign-key relationships and permissions, then writes prepared SQL.
 */
function handle_management_request(string $action,string $method,array $data): void {
    if(!in_array($action,['manage_list','manage_save','manage_delete','manage_detail','bill_list','bill_save','bill_bulk_create','bill_delete'],true))return;
    $module=str_starts_with($action,'bill_')?'assessments':(string)($_GET['module']??$data['module']??'');$permission=management_permission($module);
    if($permission==='')respond(['success'=>false,'error'=>'Unknown management module.'],404);
    $operator=require_permission($permission);$pdo=db();
    // Bill Management has its own endpoints because bills are always linked
    // to an assessment and must recalculate that assessment after changes.
    if($action==='bill_list'){
        if($method!=='GET')respond(['success'=>false,'error'=>'Method not allowed.'],405);
        respond(['success'=>true,'data'=>assessment_bills($pdo)]);
    }
    if($action==='bill_save'){
        if($method!=='POST')respond(['success'=>false,'error'=>'Method not allowed.'],405);
        csrf_check();$id=(int)($data['id']??0);$assessmentId=(int)($data['assessment_id']??0);$code=strtoupper(management_text($data,'bill_code',40));$name=management_text($data,'bill_name');$amount=(float)($data['amount']??0);
        if($assessmentId<1||$amount<=0)respond(['success'=>false,'error'=>'Select an assessment and enter a bill amount greater than zero.'],422);
        $check=$pdo->prepare('SELECT id FROM assessments WHERE id=?');$check->execute([$assessmentId]);if(!$check->fetchColumn())respond(['success'=>false,'error'=>'The selected assessment does not exist.'],422);
        $oldAssessmentId=0;$pdo->beginTransaction();
        try{
            if($id){$q=$pdo->prepare('SELECT assessment_id FROM assessment_bills WHERE id=? FOR UPDATE');$q->execute([$id]);$oldAssessmentId=(int)$q->fetchColumn();if(!$oldAssessmentId)respond(['success'=>false,'error'=>'Bill record not found.'],404);
                $pdo->prepare('UPDATE assessment_bills SET assessment_id=?,bill_code=?,bill_name=?,amount=?,due_date=?,notes=?,is_active=? WHERE id=?')->execute([$assessmentId,$code,$name,$amount,($data['due_date']??'')?:null,trim((string)($data['notes']??''))?:null,!empty($data['is_active'])?1:0,$id]);
            }else{$pdo->prepare('INSERT INTO assessment_bills(assessment_id,bill_code,bill_name,amount,due_date,notes,is_active,created_by) VALUES(?,?,?,?,?,?,?,?)')->execute([$assessmentId,$code,$name,$amount,($data['due_date']??'')?:null,trim((string)($data['notes']??''))?:null,!empty($data['is_active'])?1:0,$operator['id']]);$id=(int)$pdo->lastInsertId();}
            if($oldAssessmentId&&$oldAssessmentId!==$assessmentId)recalculate_assessment_total($pdo,$oldAssessmentId);
            recalculate_assessment_total($pdo,$assessmentId);$pdo->commit();
        }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
        audit('assessment_bill.save','assessment_bills',$id,['assessment_id'=>$assessmentId]);respond(['success'=>true,'data'=>['id'=>$id]]);
    }
    // Batch billing creates a separate assessment and linked bill for every
    // registered student, optionally limited to one academic program.
    if($action==='bill_bulk_create'){
        if($method!=='POST')respond(['success'=>false,'error'=>'Method not allowed.'],405);
        csrf_check();$code=strtoupper(management_text($data,'bill_code',40));$name=management_text($data,'bill_name');$assessmentDescription=$name;$amount=(float)($data['amount']??0);
        if($amount<=0)respond(['success'=>false,'error'=>'Enter a bill amount greater than zero.'],422);
        $where='sp.is_active=1';$params=[];
        $program=(int)($data['program_id']??0);$scope='all';if($program>0){$scope='program';$where.=' AND sp.program_id=?';$params[]=$program;}
        $q=$pdo->prepare("SELECT sp.id FROM student_profiles sp WHERE $where ORDER BY sp.id");$q->execute($params);$studentIds=$q->fetchAll(PDO::FETCH_COLUMN);
        if(!$studentIds)respond(['success'=>false,'error'=>'No active students match the selected target.'],422);
        $due=($data['due_date']??'')?:null;$schoolYear=management_text($data,'school_year',20);$semester=management_text($data,'semester',20);$notes=trim((string)($data['notes']??''))?:null;
        $pdo->beginTransaction();try{$assessmentInsert=$pdo->prepare('INSERT INTO assessments(student_profile_id,description,amount,amount_paid,due_date,school_year,semester) VALUES(?,?,0,0,?,?,?)');$billInsert=$pdo->prepare('INSERT INTO assessment_bills(assessment_id,bill_code,bill_name,amount,due_date,notes,is_active,created_by) VALUES(?,?,?,?,?,?,1,?)');$created=0;foreach($studentIds as $studentId){$assessmentInsert->execute([(int)$studentId,$assessmentDescription,$due,$schoolYear,$semester]);$assessmentId=(int)$pdo->lastInsertId();$billInsert->execute([$assessmentId,$code,$name,$amount,$due,$notes,$operator['id']]);recalculate_assessment_total($pdo,$assessmentId);$created++;}$pdo->commit();}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
        audit('assessment_bill.bulk_create','assessment_bills',null,['scope'=>$scope,'student_count'=>$created,'bill_code'=>$code]);respond(['success'=>true,'data'=>['created'=>$created]]);
    }
    if($action==='bill_delete'){
        if($method!=='POST')respond(['success'=>false,'error'=>'Method not allowed.'],405);
        csrf_check();$id=(int)($data['id']??0);if($id<1)respond(['success'=>false,'error'=>'Select a bill record.'],422);
        $pdo->beginTransaction();try{$q=$pdo->prepare('SELECT assessment_id FROM assessment_bills WHERE id=? FOR UPDATE');$q->execute([$id]);$assessmentId=(int)$q->fetchColumn();if(!$assessmentId)respond(['success'=>false,'error'=>'Bill record not found.'],404);$pdo->prepare('DELETE FROM assessment_bills WHERE id=?')->execute([$id]);recalculate_assessment_total($pdo,$assessmentId);$pdo->commit();}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
        audit('assessment_bill.delete','assessment_bills',$id,['assessment_id'=>$assessmentId]);respond(['success'=>true,'data'=>['deleted'=>true]]);
    }
    // READ: table rows plus select-field options.
    if($action==='manage_list'){
        if($method!=='GET')respond(['success'=>false,'error'=>'Method not allowed.'],405);
        respond(['success'=>true,'data'=>['rows'=>management_rows($pdo,$module),'options'=>management_options($pdo)]]);
    }
    // READ: expanded details for a student, section, or subject.
    if($action==='manage_detail'){
        ...
        if($module==='subjects'){
            ...
            $q=$pdo->prepare("SELECT se.section_name,se.school_year,se.semester,sc.day_of_week,TIME_FORMAT(sc.start_time,'%h:%i %p') start_time,TIME_FORMAT(sc.end_time,'%h:%i %p') end_time,sc.room,sc.instructor FROM class_schedules sc JOIN sections se ON se.id=sc.section_id JOIN users tu ON tu.id=sc.teacher_user_id AND tu.status='active' JOIN roles tr ON tr.id=tu.role_id AND tr.role_name='Teacher' WHERE sc.subject_id=? ORDER BY se.section_name,sc.day_of_week,sc.start_time");$q->execute([$id]);respond(['success'=>true,'data'=>['record'=>$record,'schedules'=>$q->fetchAll()]]);
        }
        respond(['success'=>false,'error'=>'Details are unavailable for this module.'],404);
    }
    // CREATE, UPDATE, and DELETE below require POST plus a valid CSRF token.
    if($method!=='POST')respond(['success'=>false,'error'=>'Method not allowed.'],405);
    ...
    if($action==='manage_save'){
        if($module==='users'){
            ...
            $section=(int)($data['section_id']??0);$q=$pdo->prepare('SELECT role_name FROM roles WHERE id=?');$q->execute([$role]);$savedRole=$q->fetchColumn();if($savedRole==='Teacher'&&$section>0)$pdo->prepare('INSERT IGNORE INTO teacher_sections(teacher_user_id,section_id) VALUES(?,?)')->execute([$id,$section]);if($savedRole!=='Teacher')$pdo->prepare('DELETE FROM teacher_sections WHERE teacher_user_id=?')->execute([$id]);
            ...
            $sectionId=(int)($data['section_id']??0);$subjectId=(int)($data['subject_id']??0);$teacherId=(int)($data['teacher_user_id']??0);if($sectionId<1||$subjectId<1||$teacherId<1)respond(['success'=>false,'error'=>'Select a section, subject, and registered teacher.'],422);
            $q=$pdo->prepare("SELECT COALESCE(NULLIF(TRIM(CONCAT_WS(' ',up.first_name,up.middle_name,up.last_name)),''),u.username) teacher_name FROM users u JOIN roles r ON r.id=u.role_id LEFT JOIN user_profiles up ON up.user_id=u.id WHERE u.id=? AND r.role_name='Teacher' AND u.status='active'");$q->execute([$teacherId]);$teacherName=$q->fetchColumn();if(!$teacherName)respond(['success'=>false,'error'=>'The selected teacher does not have an active Teacher account.'],422);
            $values=[$sectionId,$subjectId,$teacherId,$day,$start,$end,management_text($data,'room',80),(string)$teacherName];
            ...
            $q=$pdo->prepare('SELECT COUNT(*) FROM class_schedules WHERE section_id=? AND day_of_week=? AND id<>? AND start_time<? AND end_time>?');$q->execute([$sectionId,$day,$id,$end,$start]);if((int)$q->fetchColumn()>0)respond(['success'=>false,'error'=>'This section already has a class during the selected time.'],409);
            if($id){$values[]=$id;$pdo->prepare('UPDATE class_schedules SET section_id=?,subject_id=?,teacher_user_id=?,day_of_week=?,start_time=?,end_time=?,room=?,instructor=? WHERE id=?')->execute($values);}else{$pdo->prepare('INSERT INTO class_schedules(section_id,subject_id,teacher_user_id,day_of_week,start_time,end_time,room,instructor) VALUES(?,?,?,?,?,?,?,?)')->execute($values);$id=(int)$pdo->lastInsertId();}
            $pdo->prepare('INSERT IGNORE INTO teacher_sections(teacher_user_id,section_id) VALUES(?,?)')->execute([$teacherId,$sectionId]);
