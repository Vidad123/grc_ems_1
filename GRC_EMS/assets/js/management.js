/**
 * Shared administration CRUD controller.
 * It uses the configuration below to build module pages and forms. The backend
 * management controller remains responsible for validation, RBAC, and MySQL writes.
 */
(function () {
  const modules = [
    "students",
  ...
  // Field and column definitions keep each management module on its own page.
  const configs = {
    students: {
    ...
    users: {
      ...
      columns: [
        ...
        ["Email", (r) => r.email],
        ["Role", (r) => r.role_name],
        ["Assigned sections", (r) => r.assigned_sections || "—"],
        ["Status", (r) => r.status],
        [
          "Password change",
      ...
      fields: [
        ...
        [
          "section_id",
          "Add teacher section assignment",
          "select",
          0,
          "sections",
        ],
    ...
    assessments: {
      ...
      description:
        "Manage assessments, balances, and student receipt verification in one module.",
      add: "Add assessment",
      columns: [
    ...
    schedules: {
      ...
      fields: [
        ...
        ["room", "Room", "text", 1],
        ["teacher_user_id", "Teacher", "select", 1, "teachers"],
      ],
    },
    announcements: {
  ...
  // Standard JSON request helper for management CRUD actions.
  ...
  // Assessment/Billing requests have separate API routes from generic CRUD.
  async function paymentRequest(action, body) {
    ...
    try {
      ...
      throw Error("The server returned an invalid response. Check the Apache error log.");
    }
    if (!response.ok || !result.success)
  ...
  function paymentVerificationPanel(payments) {
    return `<section class="billing-panel" data-billing-panel="receipts"><div class="record-tools"><label class="table-search"><span>Search receipt verification</span><input type="search" id="receiptSearch" placeholder="Student, reference, method, or status…" autocomplete="off"></label><div class="record-filters"><label>Status<select id="receiptStatusFilter"><option value="">All statuses</option><option value="pending">Pending</option><option value="approved">Approved</option><option value="rejected">Rejected</option></select></label></div></div><article class="table-card" style="margin-top:12px"><div class="card-head"><div><h2>Receipt Verification</h2><p>Review student receipts. Only approval updates the recorded paid balance.</p><small id="receiptResultCount">${payments.length} result${payments.length === 1 ? "" : "s"}</small></div></div><div class="table-wrap"><table id="receiptTable"><thead><tr><th>Student</th><th>Assessment</th><th>Amount</th><th>Reference</th><th>Receipt</th><th>Status</th><th>Action</th></tr></thead><tbody>${payments.length ? payments.map((row) => `<tr data-payment-status="${escape(row.status)}"><td><b>${escape(row.student_name)}</b><small>${escape(row.student_number)}</small></td><td>${escape(row.description)}</td><td>${peso(row.amount)}</td><td>${escape(row.payment_method)}<small>${escape(row.payment_reference)}</small></td><td>${row.receipt_original_name ? `<a href="../api.php?action=payment_receipt&id=${encodeURIComponent(row.id)}" target="_blank" rel="noopener"><b>View receipt</b></a><small>${escape(row.receipt_original_name)} · ${Math.ceil(Number(row.receipt_file_size || 0) / 1024)} KB</small>` : "No receipt"}</td><td><span class="badge ${escape(row.status)}">${escape(row.status.replaceAll("_", " "))}</span></td><td>${row.status === "pending" ? `<button class="payment-review" data-id="${row.id}" data-status="approved">Approve</button> <button class="danger payment-review" data-id="${row.id}" data-status="rejected">Reject</button>` : `Processed${row.processor_notes ? `<small>${escape(row.processor_notes)}</small>` : ""}`}</td></tr>`).join("") : '<tr class="empty-row"><td colspan="7">No submitted receipts found.</td></tr>'}</tbody></table></div></article></section>`;
  }

  // Bills are the detailed fee items that make up an assessment total.
  // The backend recalculates the linked assessment whenever a bill changes.
  function billManagementPanel(bills) {
    return `<section class="billing-panel" data-billing-panel="bills"><div class="title-row compact-title"><div><h2>Bill Management</h2><p>Apply a bill to all registered students or optionally limit it to one program.</p></div><button class="primary" id="addBill">Add bill</button></div><div class="record-tools"><label class="table-search"><span>Search bills</span><input type="search" id="billSearch" placeholder="Bill code, fee, student, or assessment…" autocomplete="off"></label><div class="record-filters"><label>Status<select id="billStatusFilter"><option value="">All bills</option><option value="active">Active</option><option value="inactive">Inactive</option></select></label></div></div><article class="table-card" style="margin-top:12px"><div class="card-head"><div><h2>Assessment bill items</h2><p id="billResultCount">${bills.length} result${bills.length === 1 ? "" : "s"}</p></div></div><div class="table-wrap"><table id="billTable"><thead><tr><th>Code</th><th>Bill / Assessment</th><th>Student</th><th>Amount</th><th>Due date</th><th>Status</th><th>Actions</th></tr></thead><tbody>${bills.length ? bills.map((row) => `<tr data-bill-status="${Number(row.is_active) ? "active" : "inactive"}"><td><b>${escape(row.bill_code)}</b></td><td><b>${escape(row.bill_name)}</b><small>${escape(row.assessment_description)}</small></td><td><b>${escape(row.student_name)}</b><small>${escape(row.student_number)}</small></td><td>${peso(row.amount)}</td><td>${escape(row.due_date || "—")}</td><td><span class="badge ${Number(row.is_active) ? "approved" : "rejected"}">${Number(row.is_active) ? "Active" : "Inactive"}</span></td><td><button class="review edit-bill" data-id="${row.id}">Edit</button> <button class="danger delete-bill" data-id="${row.id}">Delete</button></td></tr>`).join("") : '<tr class="empty-row"><td colspan="7">No bill items have been added yet.</td></tr>'}</tbody></table></div></article></section>`;
  }

  function billDialog(row, optionsData = {}) {
    const isNew = !row.id;
    const dialog = document.getElementById("actionDialog");
    const selectOptions = (items, label) => (items || []).map((item) => `<option value="${item.id}">${escape(label(item))}</option>`).join("");
    const programOptions = selectOptions(optionsData.programs, (item) => `${item.program_code} — ${item.program_name}`);
    document.getElementById("dialogTitle").textContent = isNew ? "Add bill" : "Edit bill";
    const targetFields = isNew ? `<label class="span2">Program <small>(optional)</small><select name="program_id"><option value="">All registered students</option>${programOptions}</select></label><label>School year<input name="school_year" maxlength="20" value="2026-2027" required></label><label>Semester<input name="semester" maxlength="20" value="1st" required></label>` : `<input type="hidden" name="assessment_id" value="${escape(row.assessment_id)}">`;
    document.getElementById("dialogBody").innerHTML = `<form id="billForm"><input type="hidden" name="id" value="${row.id || ""}"><div class="grid2">${targetFields}<label>Bill code<input name="bill_code" maxlength="40" value="${escape(row.bill_code || "")}" placeholder="e.g. TUITION-01" required></label><label>Bill amount<input type="number" name="amount" step="0.01" min="0.01" value="${escape(row.amount ?? "")}" required></label><label class="span2">Bill name<input name="bill_name" maxlength="190" value="${escape(row.bill_name || "")}" placeholder="e.g. Tuition fee" required></label><label>Due date<input type="date" name="due_date" value="${escape(row.due_date || "")}"></label>${isNew ? "" : `<label class="checkbox-field"><input type="checkbox" name="is_active" value="1" ${Number(row.is_active) ? "checked" : ""}><span>Active bill</span></label>`}<label class="span2">Notes<textarea name="notes" rows="3" maxlength="5000" placeholder="Optional internal notes">${escape(row.notes || "")}</textarea></label></div><p class="form-help">This creates an individual assessment and bill for each matching registered student. Payments and balances remain separate.</p><div class="dialog-actions"><button type="button" class="secondary" id="cancelBill">Cancel</button><button class="primary" type="submit">${isNew ? "Apply bill" : "Save changes"}</button></div><div id="billMessage" role="alert"></div></form>`;
    dialog.showModal();
    document.getElementById("cancelBill").onclick = () => dialog.close();
    document.getElementById("billForm").onsubmit = async (event) => {
      event.preventDefault();
      const form = event.currentTarget;
      const button = form.querySelector("[type=submit]");
      const payload = Object.fromEntries(new FormData(form));
      if (form.elements.is_active) payload.is_active = form.elements.is_active.checked;
      try {
        button.disabled = true;
        button.textContent = "Saving…";
        const result = await paymentRequest(isNew ? "bill_bulk_create" : "bill_save", payload);
        dialog.close();
        if (result.created) alert(`Bill applied to ${result.created} student${result.created === 1 ? "" : "s"}.`);
        await load("assessments");
      } catch (error) {
        document.getElementById("billMessage").className = "error";
        document.getElementById("billMessage").textContent = error.message;
        button.disabled = false;
        button.textContent = isNew ? "Create bill" : "Save changes";
      }
    };
  }

  function bindBillManagement(bills, optionsData) {
    document.getElementById("addBill").onclick = () => billDialog({}, optionsData);
    document.querySelectorAll(".edit-bill").forEach((button) => (button.onclick = () => billDialog(bills.find((row) => String(row.id) === button.dataset.id), optionsData)));
    document.querySelectorAll(".delete-bill").forEach((button) => (button.onclick = async () => {
      const row = bills.find((item) => String(item.id) === button.dataset.id);
      if (!confirm(`Delete bill \"${row.bill_name}\"? The linked assessment total will be recalculated.`)) return;
      try { button.disabled = true; await paymentRequest("bill_delete", { id: Number(row.id) }); await load("assessments"); }
      catch (error) { button.disabled = false; alert(error.message); }
    }));
    const search = document.getElementById("billSearch"), filter = document.getElementById("billStatusFilter"), rows = [...document.querySelectorAll("#billTable tbody tr:not(.empty-row)")], count = document.getElementById("billResultCount");
    const apply = () => { const query = search.value.trim().toLowerCase(), status = filter.value; let visible = 0; rows.forEach((row) => { const match = (!query || row.textContent.toLowerCase().includes(query)) && (!status || row.dataset.billStatus === status); row.hidden = !match; if (match) visible++; }); count.textContent = `${visible} result${visible === 1 ? "" : "s"}`; };
    search.addEventListener("input", apply); filter.addEventListener("change", apply);
  }

  function bindPaymentVerification() {
    document.querySelectorAll(".payment-review").forEach((button) => {
      button.onclick = async () => {
        const notes = prompt(`Optional ${button.dataset.status} notes:`) || "";
        if (!confirm(`${button.dataset.status === "approved" ? "Approve" : "Reject"} this receipt?`)) return;
        try {
          button.disabled = true;
          await paymentRequest("payment_process", {
    ...
    const statusFilter = document.getElementById("receiptStatusFilter");
    const rows = [...document.querySelectorAll("#receiptTable tbody tr:not(.empty-row)")];
    const count = document.getElementById("receiptResultCount");
    const applyReceiptFilters = () => {
      ...
      rows.forEach((row) => {
        ...
        const matchesStatus = !selectedStatus || row.dataset.paymentStatus === selectedStatus;
    ...
    search.addEventListener("input", applyReceiptFilters);
  ...
  function bindBillingTabs() {
    document.querySelectorAll("[data-billing-tab]").forEach((button) => {
      button.onclick = () => {
        const selected = button.dataset.billingTab;
        document.querySelectorAll("[data-billing-tab]").forEach((tab) =>
          tab.classList.toggle("active", tab.dataset.billingTab === selected),
        );
        document.querySelectorAll("[data-billing-panel]").forEach((panel) =>
          panel.classList.toggle("active", panel.dataset.billingPanel === selected),
        );
        document.getElementById("addRecord").hidden = selected !== "assessments";
      };
    });
  }
  function optionLabel(key, item) {
    ...
    if (key === "teachers") return `${item.name} — ${item.email}`;
  ...
  function bindRecordTools(module, data, content) {
    ...
    const search = document.getElementById("recordSearch"),
      ...
      rows = [...content.querySelectorAll("#managementRecords tbody tr")],
  ...
  async function load(module) {
    ...
    try {
      const data = await request("manage_list", module),
        payments = module === "assessments" ? await paymentRequest("payment_list") : [],
        bills = module === "assessments" ? await paymentRequest("bill_list") : [],
        tools = recordTools(module, data),
        details = ["students", "sections", "subjects"].includes(module);
      const billingTabs = module === "assessments" ? `<nav class="billing-tabs" aria-label="Assessment and billing views"><button type="button" class="active" data-billing-tab="assessments"><span aria-hidden="true">₱</span> Assessment &amp; Billing</button><button type="button" data-billing-tab="bills"><span aria-hidden="true">☷</span> Bill Management</button><button type="button" data-billing-tab="receipts"><span aria-hidden="true">▤</span> Receipt Verification</button></nav>` : "";
      const assessmentOpen = module === "assessments" ? '<section class="billing-panel active" data-billing-panel="assessments">' : "";
      const assessmentClose = module === "assessments" ? "</section>" : "";
      content.innerHTML = `<div class="title-row"><div><h1>${escape(config.title)}</h1><p>${escape(config.description)}</p></div><button class="primary" id="addRecord">${escape(config.add)}</button></div>${billingTabs}${assessmentOpen}${tools}<article id="managementRecords" class="table-card" style="margin-top:${tools ? "12" : "24"}px"><div class="card-head"><div><h2>${module === "assessments" ? "Assessment Records" : "Records"}</h2><p id="managementResultCount">${data.rows.length} result${data.rows.length === 1 ? "" : "s"}</p></div></div><div class="table-wrap"><table><thead><tr>${config.columns.map((column) => `<th>${escape(column[0])}</th>`).join("")}<th>Actions</th></tr></thead><tbody>${data.rows.length ? data.rows.map((row) => `<tr>${config.columns.map((column) => `<td>${escape(column[1](row))}</td>`).join("")}<td>${details ? `<button class="review view-record" data-id="${row.id}">View</button> ` : ""}<button class="review edit-record" data-id="${row.id}">Edit</button> <button class="danger delete-record" data-id="${row.id}">Delete</button></td></tr>`).join("") : `<tr><td colspan="${config.columns.length + 1}">No records found.</td></tr>`}</tbody></table></div></article>${assessmentClose}${module === "assessments" ? `${billManagementPanel(bills)}${paymentVerificationPanel(payments)}` : ""}`;
      ...
      if (module === "assessments") {
        bindBillingTabs();
        bindPaymentVerification();
        bindBillManagement(bills, data.options);
      }
    } catch (error) {
  ...
  window.EMS_MANAGEMENT = {
    canHandle: (module, role) => {
      ...
      if (role === "Staff" && module !== "assessments") return false;
